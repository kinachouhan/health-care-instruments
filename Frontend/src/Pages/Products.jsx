...
import categoryData from "../Jsondata/category.json"; // your JSON data
...
export const Products = () => {
    ...
    const pageDescription = useMemo(() => {
        if (subCategoryParam) {
            // Use subcategory description if you want (optional)
            const parentCategory = categoryData.find(c => c.sub?.some(s => s.name === subCategoryParam));
            const subData = parentCategory?.sub?.find(s => s.name === subCategoryParam);
            return subData?.description
                ? subData.description
                : `Explore premium ${subCategoryParam} dental products.`;
        } else if (categoryParam) {
            const catData = categoryData.find(c => c.name === categoryParam);
            return catData?.description
                ? catData.description
                : `Browse high quality ${categoryParam} dental equipment and supplies.`;
        } else {
            return "";
        }
    }, [categoryParam, subCategoryParam]);
                                    ...
                                    selectedCategories.includes(cat.name) && (
                                        <div className="ml-6 mt-2 space-y-2">
                                            {cat.subcategories.map((subName) => {
                                                const subData = categoryData
                                                    .find((c) => c.name === cat.name)
                                                    ?.sub.find((s) => s.name === subName);
                                                return (
                                                    <label
                                                        key={subName}
